...
#define QUADFORM(dst, i0, i1)                   \
    do                                          \
    {                                           \
        mpz_mul(dst, a[i0], a[i1]);             \
        mpz_mul(tmp, b[i0], b[i1]);             \
        if (exponent < 0)                       \
            mpz_mul_2exp(dst, dst, -exponent);  \
        if (exponent > 0)                       \
            mpz_mul_2exp(tmp, tmp, exponent);   \
...
qlattice_basis::qlattice_basis(las_todo_entry const &doing, double skew) : doing(doing)
{
