...
{ /*{{{*/
    ...
    {
        __m128i x0, x1, x2, x3;
        __asm__ __volatile__("prefetcht0 0x1000(%0)\n"
                             "prefetcht0 0x1000(%1)\n"
                             "movdqa (%0),%2\n"
                             "movdqa 0x10(%0),%3\n"
                             "movdqa 0x20(%0),%4\n"
...
void process_bucket_region_run::cofactoring_sync(survivors_t &survivors) /*{{{*/
{
    ...
    for (size_t i_surv = 0; i_surv < survivors.size(); i_surv++)
    {
        ...
        if (cur.trace_on_spot())
            verbose_output_print(TRACE_CHANNEL, 0, "# about to start cofactorization for (%" PRId64 ",%" PRIu64 ")  %zu %u\n", cur.a, cur.b, x, Sx[x]);
        ...
        if (ws.las.batch || ws.las.batch_print_survivors.filename)
        {
            /* see above */
            rep.reports++;
            if (ws.conf.sublat_bound && !cur.ab_coprime())
                continue;
