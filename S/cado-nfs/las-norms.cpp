...
double_poly_reverse_scale(double_poly_ptr u, double_poly_srcptr f, const double scale)
{
    ...
    int d = f->deg;
    u->coeff[d] = f->coeff[d];
    for (double h = scale; d--; h *= scale)
        u->coeff[d] = f->coeff[d] * h;
...
#define LOGNORM_COMMON_HANDLE_ORIGIN()                                        \
    do                                                                        \
    {                                                                         \
        bool has_haxis = !j0;                                                 \
 ...
 */
int sieve_range_adjust::sieve_info_update_norm_data_Jmax(bool keep_logI)
{
...
B:=[bestrep(a):a in {{a*b*c*x:a in {1,-1},b in {1,d},c in {1,s}}:x in MM}];
  &cat [prepr(x):x in B];
    ...
    adjust_message << " (+" << std::fixed << std::setprecision(2) << 100.0 * (best_sum / reference - 1) << "%)";
